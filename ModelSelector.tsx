...
const getModelOptions = (t: (key: string) => string) => ({
    'universum-1.7': {
        id: 'universum-1.7' as ModelType,
        name: t('modelUniversum1_7Name'),
        tag: t('modelUniversum1_7Tag'),
        description: t('modelUniversum1_7Description'),
        icon: (props: any) => <SparklesIcon {...props} />,
    },
    'universum-1.7-schnell': {
        id: 'universum-1.7-schnell' as ModelType,
        name: t('modelUniversum1_7SchnellName'),
        tag: t('modelUniversum1_7SchnellTag'),
        description: t('modelUniversum1_7SchnellDescription'),
...
export const ModelSelector: FC<ModelSelectorProps> = ({ model, onModelChange, disabled, label }) => {
  ...
  const selectedOption = modelOptions[model] || modelOptions['universum-1.7'];
    ...
    <div ref={wrapperRef} className="relative">
      ...
      {isOpen && !disabled && (
        <div className={`absolute left-0 w-80 bg-base-100/80 dark:bg-dark-base-300/80 backdrop-blur-md rounded-lg shadow-2xl border border-base-300 dark:border-dark-base-200 z-20 animate-fade-in max-h-[70vh] overflow-y-auto ${direction === 'up' ? 'bottom-full mb-2' : 'top-full mt-2'}`} style={{animationDuration: '0.1s'}}>
          <ul className="p-1">
            {[
                modelOptions['universum-1.7'],
                modelOptions['universum-1.7-schnell']
